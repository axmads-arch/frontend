...
const PAY: Record<string, string> = { cash: 'Naqd', click: 'Click', payme: 'Payme', card: 'Karta' };
const DEL: Record<string, string> = { delivery: 'Yetkazib berish', pickup: 'Olib ketish' };
...
export default function Orders({ user, onAuthRequired, fmt, onReorder }: OrdersProps) {
      ...
      {loading ? (
        ...
        <div className="orders-list">
          {orders.map(o => {
            ...
            const canReorder = !!onReorder && o.items && o.items.length > 0;
            const justReordered = reorderedId === o.id;
            return (
              <div key={o.id} className="order-card">
                ...
                {canReorder && (
                  <button
                    ...
                    style={{
                      width: '100%', marginTop: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7,
                      background: justReordered ? 'var(--green)' : 'var(--green-soft)',
                      color: justReordered ? '#fff' : 'var(--green)',
                  ...
                  >
                    {justReordered ? (
                      ...
                      <><ReorderIcon /> Qayta buyurtma berish</>
                    )}
                  </button>
                )}
              </div>

...
import { Ms, ClickLogo, PaymeLogo, IconRocket, IconClock } from '../components/icons';
import type { Product, CartItem, Settings, User } from '../types';
...
interface MapPickerProps {
  onConfirm: (pos: MapPos, address: string) => void;
  selectedPos: MapPos | null;
}

function MapPicker({ onConfirm, selectedPos }: MapPickerProps) {
  const mapRef = useRef<HTMLDivElement>(null);
  const mapInstanceRef = useRef<any>(null);
  const markerRef = useRef<any>(null);
  const [pos, setPos] = useState<MapPos | null>(selectedPos || null);
    ...
    const initMap = () => {
      ...
      window.ymaps.ready(() => {
        ...
        map.events.add('click', async (e: any) => {
          ...
          } catch { /* geokodlash xatosi — foydalanuvchiga xalaqit bermaydi */ }
        });
      });
    };
  ...
  const search = async () => {
    ...
    } catch { /* qidiruv topmadi — jim davom etadi */ }
    setSearching(false);
  };
...
export default function Cart({ products, cart, settings, user, onAdd, onRemove, onClearCart, onBack, onOrderSuccess, onAuthRequired, showToast, fmt }: CartProps) {
  ...
  const [payment, setPayment] = useState<'click' | 'payme' | 'card'>('click');
  ...
  const [timeSlots] = useState<TimeSlot[]>(getTimeSlots);
  ...
  const cartItems = cart
    ...
    .filter((c): c is CartItem & { product: Product } => !!c.product);
  ...
  const checkPromo = async () => {
    ...
    try {
      ...
      setPromoError('Xatolik');
    }
    setPromoLoading(false);
  };

  const handleMapConfirm = (p: MapPos, a: string) => {
    setLat(p.lat); setLng(p.lng);
    setAddress(a || `${p.lat.toFixed(5)}, ${p.lng.toFixed(5)}`);
    showToast('📍 Manzil belgilandi');
  };

  const uploadCheck = async (file: File) => {
  ...
  return (
    <div className="cart-page">
      ...
      <div style={{ margin: '0 14px 10px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'var(--green-soft)', borderRadius: 14, padding: '10px 14px' }}>
        ...
        <button
          onClick={() => {
            const nt = deliveryType === 'delivery' ? 'pickup' : 'delivery';
            setDeliveryType(nt);
            localStorage.setItem('rc_delivery_type', nt);
          }}
      ...
      <div className="cart-items">
        {cartItems.map(item => (
          <div key={item.id} className="cart-item">
            {item.product.image ? (
              <img
                className="cart-item-img" src={item.product.image} alt={item.product.name} loading="lazy" decoding="async"
                onError={e => {
      ...
      <div className="checkout-section">
        ...
        {deliveryType === 'delivery' && (
          <div className="checkout-card">
            ...
            <MapPicker onConfirm={handleMapConfirm} selectedPos={lat !== null && lng !== null ? { lat, lng } : null} />
            {lat !== null && lng !== null && (
        ...
        <div className="checkout-card">
          ...
          <div className="delivery-toggle">
            <div
              className={`delivery-opt ${scheduleType === 'now' ? 'active' : ''}`}
              onClick={() => { setScheduleType('now'); setSelectedSlot(null); }}
            ...
            <div
              className={`delivery-opt ${scheduleType === 'scheduled' ? 'active' : ''}`}
        ...
        <div className="checkout-card">
          ...
          {promoApplied ? (
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <input
                className="field-input" style={{ flex: 1, textTransform: 'uppercase', letterSpacing: '1px' }} placeholder="PROMO10"
                value={promoCode}
          ...
          {checkImage ? (
            ...
            <label style={{ display: 'block', cursor: 'pointer' }}>
              <div style={{ border: '2px dashed var(--border)', borderRadius: 14, padding: '24px 16px', textAlign: 'center', background: 'var(--cream)' }}>
                <Ms icon="photo_camera" size={36} fill={1} color="var(--text3)" />
                <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text)', marginBottom: 4, marginTop: 8 }}>
              ...
              <input
                ...
                onChange={e => { const file = e.target.files?.[0]; if (file) uploadCheck(file); }}
              />
            </label>
          )}
        </div>

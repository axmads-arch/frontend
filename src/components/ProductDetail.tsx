  ...
  return (
    <div style={{ display: 'flex', gap: 4 }}>
      {[1, 2, 3, 4, 5].map(i => (
        <div
          key={i}
          style={{ width: size, height: size, cursor: onChange ? 'pointer' : 'default' }}
...
export default function ProductDetail({ product, cart, onAdd, onRemove, onClose, isFav, onToggleFav, fmt, user }: ProductDetailProps) {
  ...
  useEffect(() => {
    fetch(`${API_URL}/rating/${product.id}`)
      ...
      .catch(() => { /* reyting yuklanmasa jim davom etadi */ });
  }, [product.id, rSuccess]);
  ...
  const submitRating = async () => {
    ...
    try {
      ...
      setRError('Xatolik yuz berdi');
    }
    setRLoading(false);
  };

  const atStockLimit = product.stock !== null && product.stock !== undefined && qty >= product.stock;
    ...
    <div className="detail-overlay">
      ...
      <div className="detail-sheet">
        <div style={{ position: 'relative' }}>
          {product.image ? (
            <img
              className="detail-img" src={product.image} alt={product.name}
              onError={e => {
            ...
            {qty > 0 ? (
              <>
                <div className="detail-qty">
                  ...
                  <button className="detail-qty-btn" onClick={() => onAdd(product)} disabled={atStockLimit}>{Ic.plus}</button>
                </div>
          ...
          <div style={{ borderTop: '1px solid var(--border)', paddingTop: 20 }}>
            ...
            {rSuccess ? (
              ...
              <>
                ...
                <button
                  onClick={submitRating} disabled={rLoading || !myStars}
